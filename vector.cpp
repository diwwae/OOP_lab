...
template<typename T>
Vector<T>::Vector(const Vector<T> &vec) : Vector(vec.arrayCapacity) {
    memcpy(Array, vec.Array, arrayCapacity * sizeof(T));
}

template<typename T>
Vector<T>::Vector(Vector<T> &&oth) noexcept
        : arrayCapacity(oth.arrayCapacity),
          arraySize(oth.arraySize),
          Array(oth.Array) {
    oth.arrayCapacity = 0;
    oth.arraySize = 0;
    oth.Array = nullptr;
...
void Vector<T>::reserve(const size_t newCapacity) {
    ...
    try {
        if (newCapacity > this->arrayCapacity) {
            std::uninitialized_copy(this->Array, this->Array + this->arraySize,
                                    newArray);
        } else {
            std::uninitialized_copy(this->Array, this->Array + newCapacity,
                                    newArray);
        }
    } catch (...) {
        delete[] reinterpret_cast<char *>(this->Array);
...
void Vector<T>::resize(const size_t newSize, const T &value) {
    ...
    if (this->arraySize > newSize) {
        for (size_t deleteValue = newSize; deleteValue < this->arraySize;
             ++deleteValue) {
    ...
    for (size_t currentValue = this->arraySize; currentValue < newSize;
         ++currentValue) {
...
T &Vector<T>::at(const size_t index) const {
    if ((index >= this->arraySize) || (index < 0)) {
        throw std::range_error("Incorrect vector index!");
    }
